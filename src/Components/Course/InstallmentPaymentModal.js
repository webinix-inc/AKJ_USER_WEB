...
const InstallmentPaymentModal = ({
        ...
        if (userId) {
          try {
            ...
            if (timeline.length > 0) {
              ...
              if (!planTotalAmount) {
                const calculatedTotal = timeline.reduce(
                  (sum, inst) => sum + (inst.amount || 0),
                  0
                );
              ...
              try {
                ...
                if (userId && courseId) {
                  ...
                  setOrdersMap((prev) => ({
                    ...prev,
                    ...fullOrdersMapForTimeline,
                  }));
                }
              } catch (ordersError) {
                console.warn(
                  "Could not fetch orders for timeline:",
                  ordersError
                );
              }
              ...
              const formattedTimeline = timeline.map((inst, idx) => {
                ...
                if (
                  ...
                  const savedInstallment =
                    userPurchasedCourse.installments.find(
                      (savedInst) =>
                        savedInst.installmentNumber === idx + 1 ||
                        (savedInst.installmentNumber ===
              ...
              if (formattedTimeline.length > 0) {
                ...
                const finalPlanType =
                  planTypeFromTimeline ||
                  planTypeFromPurchased ||
                  propPlanType ||
                  "Standard";
                ...
                if (
                  ...
                  const originalTotal = userPurchasedCourse.installments.reduce(
                    (sum, inst) => sum + (inst.amount || 0),
                    0
                  );
                  ...
                  if (!planTotalAmount) {
                    const calculatedTotal = formattedTimeline.reduce(
                      (sum, inst) => sum + (inst.amount || 0),
                      0
                    );
                    if (calculatedTotal > 0) {
                  ...
                  if (planRemainingAmount === null) {
                    const paidAmount = formattedTimeline
                      .filter((inst) => inst.isPaid)
                      .reduce((sum, inst) => sum + (inst.amount || 0), 0);
                    const calculatedRemaining =
                      (planTotalAmount || 0) - paidAmount;
                    if (calculatedRemaining >= 0) {
                      setPlanRemainingAmount(calculatedRemaining);
                    }
                  }
              ...
              const allPaid =
                ...
                formattedTimeline.every((inst) => inst.isPaid === true);
              if (allPaid) {
            ...
            console.log(
              "Timeline not available (user may not be enrolled yet), fetching plans...",
              timelineError
            );
        ...
        try {
          ...
          if (Array.isArray(plansData) && plansData.length > 0) {
            ...
            if (planTypeToUse) {
              ...
              if (foundPlan) {
                ...
                if (caseInsensitiveMatch) {
                  ...
                  console.error(
                    `❌ [InstallmentPaymentModal] Plan with planType "${planTypeToUse}" not found in available plans!`
                  );
                  console.error(
                    `❌ [InstallmentPaymentModal] Available plans:`,
                    plansData.map((p) => p.planType)
                  );
                  ...
                  if (
                    ...
                    const userFirstAmount =
                      userPurchasedCourse.installments[0]?.amount;
                    const matchingPlan = plansData.find((p) => {
                      if (
                        !p.installments ||
                        p.installments.length !==
                          userPurchasedCourse.installments.length
                      ) {
                        return false;
                      }
                    ...
                    if (matchingPlan) {
                      ...
                      console.error(
                        `❌ [InstallmentPaymentModal] Could not find matching plan even by amount!`
                      );
                      // Don't select wrong plan - return error state
                      throw new Error(
                        `Your enrolled plan (${planTypeToUse}) is not available. Please contact support.`
                      );
                    }
                  } else {
                    ...
                    console.warn(
                      `⚠️ [InstallmentPaymentModal] Plan "${planTypeToUse}" not found, using first plan as fallback: ${selectedPlan.planType}`
                    );
                  }
                }
              }
            ...
            try {
              // Use the userId we got earlier (from userData or profileData)
              const currentUserId =
                userId || userData?.userId || profileData?._id;
              ...
              console.warn(
                "Could not fetch orders, continuing with other payment sources:",
                ordersError
              );
            }
            ...
            if (
              selectedPlan.userPayments &&
              Array.isArray(selectedPlan.userPayments)
            ) {
              selectedPlan.userPayments.forEach((payment) => {
                // Compare user IDs (handle both string and ObjectId)
                const paymentUserId =
                  payment.userId?.toString?.() || payment.userId;
                const currentUserId =
                  (
                    userId ||
                    userData?.userId ||
            ...
            if (userData?.purchasedCourses) {
              enrolledUserPurchasedCourse = userData.purchasedCourses.find(
                (pc) => {
                  const courseIdStr = pc.course?.toString?.() || pc.course;
                  const currentCourseId = courseId?.toString?.() || courseId;
                  return (
            ...
            if (isEnrolledUser && userOriginalInstallments) {
              ...
              const originalTotal = userOriginalInstallments.reduce(
                (sum, inst) => sum + (inst.amount || 0),
                0
              );
            ...
            if (selectedPlan.nextDueInstallment) {
              ...
              const nextDueIdx = selectedPlan.installments.findIndex(
                (inst) =>
                  inst._id?.toString() ===
                    selectedPlan.nextDueInstallment._id?.toString() ||
            ...
            const allPaid =
              timeline.length > 0 &&
              timeline.every((inst) => inst.isPaid === true);
            if (allPaid) {
              setNextInstallment(null); // All paid - no next installment
            } else {
              setNextInstallment(unpaidInstallment);
  ...
  useEffect(() => {
    ...
    const handleTimelineUpdate = async (event) => {
      ...
      if (
        eventCourseId?.toString() === courseId?.toString() &&
        ...
        try {
          ...
          console.error(
            "Error refreshing installment details from event:",
            error
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (isOpen && hasAttemptedFetch && !loading) {
      ...
      if (hasNoData) {
        ...
        console.log(
          "✅ Installment details found, keeping modal open:",
          installmentDetails?.length,
          "items"
        );
      }
    }
  ...
  const handlePayment = async () => {
    ...
    if (!hasValidUserId(currentUserData)) {
      try {
        ...
        if (fetchedProfile && (fetchedProfile.userId || fetchedProfile._id)) {
          ...
          console.log(
            "✅ Retrieved userData from fetchUserProfile:",
            currentUserData
          );
        }
      } catch (error) {
    ...
    const userId =
      getUserId(currentUserData)?.toString?.() || getUserId(currentUserData);
    const currentCourseId = courseId?.toString?.() || courseId;
    if (!userId || !currentCourseId) {
      console.error("❌ Payment validation failed:", {
        ...
        userData: userData
          ? {
              ...
              hasPurchasedCourses: !!userData.purchasedCourses,
            }
          : null,
        courseId,
      });
      notification.error({
        ...
        }${!currentCourseId ? "Course ID missing." : ""}`,
      });
      return;
    }
    ...
    try {
      ...
      const actualInstallmentIndex =
        nextInstallment.installmentIndex !== undefined
          ? nextInstallment.installmentIndex
          : installmentDetails?.findIndex((inst) => inst === nextInstallment) ||
            0;
      ...
      if (verificationResult.success) {
        notification.success({
          ...
          description: `Your ${actualInstallmentIndex + 1}${
            actualInstallmentIndex === 0
              ? "st"
              : actualInstallmentIndex === 1
              ? "nd"
              : actualInstallmentIndex === 2
              ? "rd"
              : "th"
        ...
        try {
          ...
          console.log(
            "🗑️ [CACHE] Cleared frontend API cache for user profile after installment payment"
          );
        } catch (cacheError) {
          console.error(
        ...
        const refreshProfileWithRetry = async () => {
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
              ...
              console.error(
                `⚠️ Profile refresh attempt ${attempt} failed:`,
                error
              );
        ...
        refreshProfileWithRetry().then((success) => {
          if (success) {
            ...
            console.warn(
              "⚠️ Profile refresh failed after retries, but payment was successful"
            );
          }
        });
        ...
        if (window.dispatchEvent) {
          window.dispatchEvent(
            new CustomEvent("profileUpdated", {
              detail: {
                reason: "installment_payment_success",
                courseId: currentCourseId,
                installmentIndex: actualInstallmentIndex,
              },
            })
        ...
        if (window.dispatchEvent) {
          window.dispatchEvent(
            new CustomEvent("installmentTimelineUpdated", {
          ...
          setTimeout(async () => {
            try {
              ...
              const orderFromResponse =
                verificationResult?.order || verificationResult?.data?.order;
              const orderId =
                  ...
                  if (userId && courseId) {
                    ...
                    const userOrder = paidOrders.find((order) => {
                      ...
                      return (
                        ...
                        orderCourseId === currentCourseId &&
                        order.paymentMode === "installment" &&
                        order.installmentDetails?.installmentIndex ===
                          actualInstallmentIndex &&
                        order.status === "paid"
                      );
                    });
                    ...
                    if (userOrder) {
                      ...
                      const receiptData = {
                        ...
                        paymentDate:
                          userOrder.paidAt || userOrder.updatedAt || new Date(),
        ...
        setTimeout(async () => {
          try {
            ...
            console.error(
              "Error refreshing installment details from API:",
              error
            );
        ...
        <div className="p-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <p className="text-sm text-gray-600">
              Course Price: ₹{coursePrice?.toLocaleString("en-IN") || "N/A"}
            </p>
          </div>
          ...
          {loading ? (
            ...
            (Array.isArray(installmentDetails) &&
            ...
            <>
              ...
              {planType && (
                <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200">
                  <h3 className="text-lg font-bold text-[#023d50] mb-3">
                    {planType} Plan
                  </h3>
                  <div className="space-y-2">
                    ...
                    {planRemainingAmount !== null &&
                      planRemainingAmount !== undefined && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-lg font-semibold text-orange-600">
                            ₹
                            {Math.round(planRemainingAmount).toLocaleString(
                              "en-IN"
                            )}
                          </span>
                        </div>
              ...
              <div className="mb-6">
                <Timeline>
                  {installmentDetails.map((installment, index) => {
                    ...
                    const paymentDate =
                      installment.paymentDate || installment.paidDate;
                          ...
                          {isPaid ? (
                            <div className="text-sm mt-2">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    handleDownloadReceipt(index, installment)
                                  }
              ...
              {nextInstallment ? (
                <>
                  <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-gray-700 font-medium">
                        Next Installment to Pay:
                      </span>
                      <span className="text-2xl font-bold text-[#023d50]">
                        ₹
                        {Math.round(nextInstallment.amount).toLocaleString(
                          "en-IN"
                        )}
                      </span>
                    </div>
                    ...
                    {processing || paymentLoading ? (
                      ...
                      <>
                        ...
                        <span className="text-xl">
                          Pay Installment{" "}
                          {installmentDetails.indexOf(nextInstallment) + 1} - ₹
                          {Math.round(nextInstallment.amount).toLocaleString(
                            "en-IN"
                          )}
                        </span>
                ...
                <div className="text-center py-4 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <p className="text-sm text-gray-600">
                    You have completed all your installment payments.
                  </p>
                </div>
              )}
