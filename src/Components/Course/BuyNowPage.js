...
const BuyNowPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (_id) {
        console.log(
          "🛒 BuyNowPage: Fetching installments and subscriptions for course:",
          _id
        );
        try {
          ...
          const userPurchasedCourse = userData?.purchasedCourses?.find((pc) => {
            ...
            return (
              pcCourseId === currentCourseId && pc.paymentType === "installment"
            );
          });
          ...
          const isEnrolled =
            ...
            userPurchasedCourse.installments.length > 0;

          if (isEnrolled && userData?.userId) {
            fetchOptions.userId = userData.userId;
            console.log(
              "✅ [BuyNowPage] User is ENROLLED with plan:",
              userPurchasedCourse.planType,
              "- passing userId to filter to enrolled plan"
            );
          } else {
            console.log(
              "ℹ️ [BuyNowPage] User not enrolled yet - showing all plans for selection"
            );
          ...
          if (planType) {
            ...
            console.log(
              "✅ [BuyNowPage] Passing planType to filter plans:",
              planType
            );
          }
        ...
        console.error(
          "❌ BuyNowPage: No course ID found in location.state or URL params"
        );
    ...
    const loadProfile = async () => {
      try {
        ...
        if (!userData || !userData.userId) {
          console.log(
            "🛒 BuyNowPage: Fetching user profile for payment processing..."
          );
          await fetchUserProfile(false); // Don't force refresh unless necessary
        } else {
          console.log(
            "🛒 BuyNowPage: User data already available for payments"
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (installments.length > 0) {
      ...
      if (userData?.purchasedCourses && _id) {
        ...
        const userPurchasedCourse = userData.purchasedCourses.find((pc) => {
          const pcCourseId = pc.course?.toString?.() || pc.course;
          return (
            pcCourseId === courseIdString && pc.paymentType === "installment"
          );
        });
        ...
        if (isActuallyEnrolled && userPurchasedCourse?.planType) {
          ...
          const enrolledPlan = installments.find(
            (p) => p.planType === userPurchasedCourse.planType
          );
          if (enrolledPlan?._id && !selectedPlanId) {
            setSelectedPlanId(enrolledPlan._id.toString());
          }
          console.log(
            `✅ [BuyNowPage] User ENROLLED with plan: ${userPurchasedCourse.planType}, showing all plans`
          );
          ...
          console.log(
            `✅ [BuyNowPage] Using planType from location.state: ${planType}, showing all plans`
          );
        }
      } else if (planType && !selectedPlanId) {
        // If planType is provided in location.state, set it as selected but show all plans
      ...
      filtered.forEach((plan) => {
        if (!plan.totalAmount || !plan.numberOfInstallments) {
          console.warn(
            `⚠️ [BuyNowPage] Plan ${plan.planType} (ID: ${plan._id}) missing amount data:`,
            {
    ...
    _id,
  ]); // 🔥 Removed selectedPlanId - don't refilter when plan is selected
    ...
    if (subscriptions.length > 0 && _id) {
      ...
      if (filteredSubscriptions.length > 0) {
        ...
        console.log("⚠️ BuyNowPage: No subscriptions found for course ID:", {
          ...
          subscriptionCourseIds: subscriptions.map(
            (sub) => sub?.course?._id?.toString?.() || sub?.course?._id
          ),
        });
      }
    } else {
    ...
    setTimeout(async () => {
      try {
        const receiptElement =
          receiptContainer.querySelector(".payment-receipt");
        if (!receiptElement) {
        ...
        // Generate filename
        const filename = `Payment_Receipt_${receiptData.courseTitle.replace(
          /\s+/g,
          "_"
  ...
  const handlePayment = async ({
    ...
    try {
      ...
      try {
        ...
        const checkCourseAccess = async () => {
          try {
            const response = await api.post(
              `/stream/check-course-access/${courseId}`,
              {
                userId: userId,
              }
            );
            ...
            console.warn(
              "❌ [BuyNowPage] Error checking course access:",
              error
            );
            return false;
          }
        ...
        // Emit update event
        window.dispatchEvent(
          new CustomEvent("enrollmentUpdate", {
            detail: { courseId, enrollmentData },
          })
        );
        ...
        while (retryCount < maxRetries && !hasAccess) {
          ...
          console.log(
            `🔄 Checking course access, attempt ${retryCount}/${maxRetries}`
          );
          ...
          console.log(
            `🔍 [BuyNowPage] Access check attempt ${retryCount}/${maxRetries}: ${
        ...
        if (hasAccess) {
          ...
          try {
            ...
            console.log(
              "🗑️ [CACHE] Cleared frontend API cache for user profile"
            );
          } catch (cacheError) {
            console.error(
              "⚠️ [CACHE] Error clearing frontend cache (non-critical):",
              cacheError
            );
          }
          ...
          window.dispatchEvent(
            new CustomEvent("enrollmentCompleted", {
          ...
          try {
            ...
            console.log(
              "✅ [BuyNowPage] Refetched installments after enrollment - backend should filter to enrolled plan"
            );
          } catch (error) {
            console.error(
              "⚠️ [BuyNowPage] Error refetching installments after enrollment:",
              error
            );
          }
          ...
          console.log("📄 Setting up auto-download receipt...", {
          ...
          try {
            ...
            setTimeout(async () => {
              try {
                ...
                const orderFromResponse =
                  verificationResponse?.order ||
                  verificationResponse?.data?.order;
                const orderId =
                  orderFromResponse?.orderId ||
                  orderData?.orderId ||
                  paymentResponse?.orderId;
                const paymentId =
                  orderFromResponse?.paymentId ||
                  paymentResponse?.razorpay_payment_id ||
                  paymentResponse?.paymentId;
                const trackingNumber =
                  orderFromResponse?.trackingNumber || "N/A";
                  ...
                  try {
                    if (userId && courseId) {
                      ...
                      if (userOrder) {
                        ...
                        console.log(
                          "📄 Generating receipt PDF with data:",
                          receiptData
                        );
                    ...
                    console.error(
                      "⚠️ Error fetching order from API:",
                      fetchError
                    );
                  }
                }
                ...
                console.error(
                  "❌ Error auto-downloading receipt:",
                  receiptError
                );
          ...
          console.warn(
            "⚠️ Course access not confirmed after max retries, but payment was successful"
          );

          // Update notification with final status
          const finalMessage =
            "Access is being processed. You will be redirected...";
          setEnrollmentStatus({
            attempts: maxRetries,
            maxAttempts: maxRetries,
          ...
          localStorage.setItem(
            `enrollment-checking-${courseId}`,
            JSON.stringify(finalEnrollmentData)
          );
          ...
          if (hasAccess) {
            ...
            notification.warning({
              message: "Payment Successful!",
              description:
                "Course access is being set up. Please refresh the page if access doesn't appear.",
              placement: "top",
              duration: 8,
        ...
        notification.success({
          message: "Payment Successful!",
          description:
            "Your payment was processed. Redirecting in 5 seconds...",
          placement: "top",
          duration: 5,
          key: `payment-success-${courseId}`,
        });
  ...
  const handleBuyNow = async () => {
    ...
    const selectedPlan =
      filteredInstallments.find(
        (inst) => inst._id?.toString() === selectedPlanId?.toString()
    ...
    await handlePayment({
      ...
      userId: userData.userId,
      planType:
        selectedPlan?.planType || selectedValidityDetails?.validity
          ? `${selectedValidityDetails.validity} months`
          : null,
  ...
  const handleProceedToPay = () => {
    ...
    if (!subscription) {
      notification.error({
        ...
        description:
          "No subscription plans are available for this course. Please try refreshing the page.",
        placement: "top",
      });
      return;
    }
  ...
  function getInstallmentPrice(planType) {
    ...
    console.warn(
      `⚠️ [getInstallmentPrice] No installment found for planType: ${planType}. Available plans:`,
  ...
  function getTotalPrice(planType) {
    ...
    console.warn(
      `⚠️ [getTotalPrice] No installment found for planType: ${planType}. Available plans:`,
      installments.map((i) => i.planType)
    );
    return null; // Return null if no matching installment plan is found
  }
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="text-center max-w-md mx-auto p-8 bg-white rounded-lg shadow-lg">
          ...
          <p className="text-gray-600 mb-4">
            We couldn't load the subscription plans for this course.
          </p>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-6">
            <p className="text-sm text-yellow-800">
              ...
              <br />
              • Subscription plans not configured for this course
              <br />
      ...
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">
            Course ID is missing from the URL or navigation state.
          </p>
          <button
            onClick={() => navigate("/explorecourses")}
  ...
  const hasCourseData =
    courseFromState && (courseFromState.title || courseFromState._id);
  ...
  const calculateTotalPerMonth = (basePricePerMonth) => {
    ...
    const handlingPerMonth = parseFloat(
      calculateInternetHandlingPerMonth(basePricePerMonth)
    );
  ...
  const getDiscountedPrice = () => {
    ...
    const discountedPrice =
      selectedValidityDetails.price * (1 - discountPercentage / 100);
  ...
  const calculateInternetHandlingPerInstallment = (basePricePerInstallment) => {
    return (
      basePricePerInstallment *
      (internetHandlingPercentage / 100)
    ).toFixed(2);
  };

  // Calculate total charges per installment
  const calculateTotalPerInstallment = (basePricePerInstallment) => {
    const gstPerInstallment = parseFloat(
      calculateGSTPerInstallment(basePricePerInstallment)
    );
    const handlingPerInstallment = parseFloat(
      calculateInternetHandlingPerInstallment(basePricePerInstallment)
    );
    return (
      basePricePerInstallment +
      gstPerInstallment +
      handlingPerInstallment
    ).toFixed(2);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center gap-4">
              ...
              />
              <div>
                <h1 className="text-2xl font-bold text-[#023d50]">
                  AKJ Classes
                </h1>
                ...
                {loading ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">
                      Please wait while we fetch your course plans.
                    </p>
                  </div>
                ) : !subscription ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 mb-4">
                      {hasCourseData
                        ? `We're fetching subscription plans for "${
                            courseFromState?.title || "this course"
                          }". Please wait...`
                    ...
                    {hasCourseData && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 text-left">
                        <p className="text-sm text-blue-800">
                          ...
                          <br />• Course ID: {_id}
                          <br />• Course Title:{" "}
                          {courseFromState?.title || "N/A"}
                          <br />• Price: ₹{courseFromState?.price || "N/A"}
                          <br />• Subscriptions loaded: {subscriptions.length}
                          <br />• Filtered subscriptions:{" "}
                  ...
                  subscription.validities.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 mb-4">
                      This course has subscription data but no pricing plans
                      (validities) are configured.
                    </p>
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                      <p className="text-sm text-yellow-800">
                        <strong>Technical Info:</strong>
                        <br />• Subscription found:{" "}
                        {subscription ? "Yes" : "No"}
                        <br />• Course ID: {_id}
                        <br />• Validities array:{" "}
                        {subscription?.validities ? "Empty" : "Missing"}
                      </p>
                    </div>
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 w-full">
                      {subscription.validities.map((validity, index) => {
                        ...
                        const isSelected =
                          selectedValidity === validity.validity;
                        const isPopular = index === 1; // Make second plan popular

                        // 🔥 CRITICAL: If prices are not available, show course price as fallback
                        const displayTotalPrice =
                          totalPrice || (price ? price.toFixed(0) : "0");
                        const displayInstallmentPrice =
                          installmentPrice ||
                          (price && validity.validity
                            ? (price / validity.validity).toFixed(0)
                            : null);
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              console.log(
                                "Course price from subscription:",
                              ...
                              if (matchingPlan?._id) {
                                setSelectedPlanId(matchingPlan._id.toString());
                                console.log(
                                  `✅ [BuyNowPage] Selected plan ID: ${matchingPlan._id} for ${validity.validity} months`
                                );
                              } else {
                                console.warn(
                                  `⚠️ [BuyNowPage] No matching plan found for ${validity.validity} months`
                                );
                              }

                              if (appliedCoupon) {
                                calculateDiscountOnCoupon(
                                  totalPrice || price || 0
                                );
                              }
                            }}
                          ...
                          >
                            ...
                            <div className="text-center mb-6">
                              <h3 className="text-xl font-bold text-[#023d50] mb-4">
                                {validity.validity} Months
                              </h3>
                              ...
                              <div className="text-center">
                                ...
                                {oldPrice &&
                                  oldPrice > parseFloat(displayTotalPrice) && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 space-y-3 border border-blue-200">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-gray-600">
                          Handling ({internetHandlingPercentage}%)
                        </span>
                        <span className="font-medium text-gray-800">
                          ₹
                          {calculateInternetHandlingPerInstallment(
                            calculateDiscountedPricePerInstallment()
                          )}
                        </span>
                      </div>
                      ...
                      <div className="border-t pt-2 mt-2">
                        ...
                        {getSelectedInstallmentPlan() && (
                          <div className="text-xs text-gray-500 mt-1 text-center">
                            {getSelectedInstallmentPlan().numberOfInstallments}{" "}
                            Installments
                          </div>
                        )}
                      </div>
                  ...
                  <div className="border-t pt-4">
                    <div className="flex justify-between items-center mb-4">
                      <span className="text-sm font-semibold text-gray-700">
                        Total ({selectedValidityDetails.validity} months)
                      </span>
                        ...
                        {appliedCoupon ? (
                          <>
                            <span className="text-xl font-bold text-[#023d50]">
                              ₹
                              {(total - appliedCoupon.currentDiscount).toFixed(
                                0
                              )}
                            </span>
            ...
            <div className="p-6">
              ...
              {installments.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">
                    Installment plans are not configured for this duration.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {installments.map((installmentPlan) => {
                    ...
                    if (userData?.purchasedCourses) {
                      userPurchasedCourse = userData.purchasedCourses.find(
                        (pc) => {
                          const courseIdStr =
                            pc.course?.toString?.() || pc.course;
                          const currentCourseId =
                    ...
                    const baseInstallments =
                      isEnrolledUser && userPurchasedCourse?.installments
                        ? userPurchasedCourse.installments.map(
                            (userInst, idx) => {
                              const planInst =
                                installmentPlan.installments[idx] || {};
                              return {
                                amount: userInst.amount, // Use original amount
                                dueDate:
                                  planInst.dueDate ||
                                  `DOP + ${idx} month${idx > 1 ? "s" : ""}`,
                    ...
                    const installmentsWithUserStatus = baseInstallments.map(
                      (installment, index) => {
                    ...
                    if (isEnrolledUser && userPurchasedCourse?.installments) {
                      ...
                      planTotal = userPurchasedCourse.installments.reduce(
                    ...
                    const firstUnpaidInstallment =
                      installmentsWithUserStatus.find(
                        (installment) => !installment.isPaid
                      );

                    const isSelected =
                      selectedPlanId === installmentPlan._id?.toString();
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-[#023d50]">
                            {installmentPlan.planType} Plan
                          </h3>
                              ...
                              planRemaining !== null && (
                                <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                                  ...
                                  <span className="text-lg font-semibold text-orange-600">
                                    ₹
                                    {Math.round(planRemaining).toLocaleString(
                                      "en-IN"
                                    )}
                                  </span>
                                </div>
                        ...
                        <div className="mb-4 space-y-2">
                          {installmentsWithUserStatus.map(
                            (installment, index) => (
                              <div
                                key={index}
                        ...
                        {!isSelected ? (
                          <button
                            onClick={() => {
                              ...
                              console.log(
                                `✅ [BuyNowPage] Plan selected: ${installmentPlan.planType} (ID: ${installmentPlan._id})`
                              );
                              // Save to localStorage for persistence
                              localStorage.setItem(
                                `selectedPlan_${_id}`,
                                installmentPlan._id.toString()
                              );
                              localStorage.setItem(
                                `selectedPlanType_${_id}`,
                                installmentPlan.planType
                              );
                            }}
