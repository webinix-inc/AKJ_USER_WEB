...
const PurchasedCoursesCarousel = () => {
  ...
  useEffect(() => {
    ...
    const showEnrollmentNotification = (courseId, enrollmentData) => {
      ...
      const progressPercent = Math.round(
        (enrollmentData.attempts / enrollmentData.maxAttempts) * 100
      );
          ...
          <div>
            ...
            {enrollmentData.attempts > 0 && (
              <div className="text-xs text-gray-500 mt-1">
                Checking access... ({enrollmentData.attempts}/
                {enrollmentData.maxAttempts})
              </div>
            )}
          </div>
        ),
  ...
  useEffect(() => {
    const fetchPurchasedCourses = async () => {
      try {
        ...
        console.log(
          "✅ [My Courses] Purchased courses response:",
          response.data
        );
        ...
        if (response.data && response.data.data) {
          ...
          console.log(
            `📚 [My Courses] Found ${response.data.data.length} purchased courses`
          );
          console.log(
            "🔍 [My Courses] Course types:",
        ...
        console.error(
          "❌ [My Courses] Error fetching purchased courses:",
          error
        );
        setError(
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-[#023d50] font-semibold">
  ...
  return (
    <div className="w-full max-w-full animate-apple-fade-in">
      {/* Hero Section - Light Theme */}
      <div className="relative gradient-apple-primary compact-hero rounded-apple-xl mb-4 shadow-apple mx-4 mt-4 border border-apple-gray-200">
      ...
      <div className={`w-full ${isMobile ? "px-4 pb-12" : "px-6"}`}>
        {purchasedCourses.length === 0 ? (
          <div className="card-apple p-8 text-center shadow-apple">
            ...
            <h3 className="app-body font-bold text-brand-primary mb-3 font-apple">
              No Courses Yet
            </h3>
          ...
          <div className="card-apple p-6 shadow-apple">
            <div className="mb-6">
              <h2 className="app-body font-bold text-brand-primary mb-2 font-apple">
                Your Learning Dashboard
              </h2>
              <p className="app-caption text-apple-gray-600">
              ...
              <div className="flex flex-col gap-4 pb-8">
                {purchasedCourses.map((course, index) => (
                  <div
                    ...
                    className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 ease-out overflow-x-hidden cursor-pointer border border-gray-100 hover:border-teal-200"
                    onClick={() => handleExploreClick(course)}
                  >
                    <div className="relative h-48 overflow-x-hidden rounded-t-xl">
                      <img
              ...
              >
                {purchasedCourses.map((course, index) => (
                  <SwiperSlide key={course._id || index}>
                    <div
                      className="group bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 ease-out overflow-x-hidden cursor-pointer transform hover:scale-[1.02] border border-gray-100 hover:border-teal-200"
                      onClick={() => handleExploreClick(course)}
                    >
                      <div className="relative h-48 overflow-x-hidden rounded-t-xl">
                        <img
