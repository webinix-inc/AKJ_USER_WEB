...
const Register = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (error.type === 'USER_EXISTS') {
        errorMessage = "User with this phone number already exists. Redirecting to login...";
        ...
        errorMessage = error.message || "Server error occurred. Please try again later.";
        
        // If the error message indicates partial success, still try to proceed
        if (error.message && error.message.includes('Registration successful but')) {
          toast.warning(errorMessage);
          // Try to extract user data from error response if available
          if (error.response?.data?.data?.id) {
            console.log('🔄 Partial success - proceeding with OTP verification');
  ...
  return (
    <div className="min-h-screen font-apple flex flex-col items-center justify-center" style={{background: 'linear-gradient(135deg, #fafafa 0%, #f5f5f5 50%, #f0f0f0 100%)'}}>
      <div className="flex flex-col md:flex-row card-apple rounded-apple-2xl shadow-apple-lg overflow-hidden w-full max-w-6xl mx-4">
        {/* Left Side - Hero Section */}
        <div className="relative h-auto w-full md:w-1/2 flex flex-col items-center justify-center gradient-apple-accent">
          <button
            onClick={() => navigate("/")}
            className="absolute top-6 right-6 z-10 w-10 h-10 glass-apple-dark rounded-full flex items-center justify-center hover:bg-white/30 transition-all duration-300 hover-lift"
            aria-label="Go back to home"
          >
            <CgCloseR color="white" size={24} />
          </button>
          
          <div className="text-center text-white p-8 animate-apple-slide-up">
            <h2 className="app-title text-white mb-4">Join Us Today!</h2>
            <p className="app-body text-orange-100 mb-8">Start your learning journey with expert guidance</p>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center items-center">
          <div className="text-center mb-8 w-full max-w-md animate-apple-slide-up">
            <h3 className="app-title text-brand-primary mb-3">Create Account</h3>
          ...
          <div className="w-full flex flex-col items-center gap-6 mb-8 max-w-md animate-apple-fade-in">
            <div className="w-full">
              <label className="block app-caption font-medium text-brand-primary mb-2">First Name</label>
              <Input
            ...
            <div className="w-full">
              <label className="block app-caption font-medium text-brand-primary mb-2">Last Name</label>
              <Input
            ...
            <div className="w-full">
              <label className="block app-caption font-medium text-brand-primary mb-2">Mobile Number</label>
              <PhoneInput
                country={"in"}
                value={phone}
